(: Appends Wiktionary glosses (key 'wikt/<lemma>') to the shared store, on top of
   ...
   the 'wikt/' prefix is load-bearing: it keeps this keyspace disjoint from the
   bare Greek lemma keys (LSJ shortdefs) and the tlg keys (work metadata added by
   seed/index.xq). Without it, these writes would overwrite the LSJ shortdefs and
   pollute the blind store:keys sweeps in seed/freq.xq and seed/formulae.xq.

   Run after shortdefs (which clears the store) and before index (which writes
   it out as 'glaux'). :)
